...
const UserBlock = () => {
  ...
  useEffect(() => {
    setIsBtnShown(userData.name !== user.name || userData.email !== user.email ? true : false)
  }, [userData, user])
  ...
  return (
    <form onSubmit={onSaveClick} className="mb-20`">
      ...
      <div className="mb-6">
        <Input onChange={(name) => handleChange(name)} name={'email'} value={userData.email} placeholder="Логин" />
      </div>
      <div className="mb-6">
        <PasswordInput onChange={(name) => handleChange(name)} name={'password'} value={userData.password} placeholder="Пароль"  />
      </div>
      {isErrorShown && <p className={`text text_type_main-small ${styles.error_info}`}>Данные не заполнены</p>}
